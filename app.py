...
# Initialize the OLA Maps client using API key from environment variable
client = Client(api_key=os.getenv("OLAMAPS_API_KEY"))

# Set up the Gemini API key and initialize the model
...
def get_location_details(latitude, longitude):
    try:
        reverse_geocode_results = client.reverse_geocode(lat=latitude, lng=longitude)
...
def chat():
    if request.method == 'POST':
        ...
        try:
            ...
            response = f"Error fetching data from Gemini API: {str(e)}"
...
def map_view():
    ...
    return render_template('map.html', latitude=latitude, longitude=longitude, location_details=location_details)
